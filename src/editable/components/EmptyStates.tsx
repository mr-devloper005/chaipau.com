...
export function EmptyState({
  ...
  return (
    <section className={cn('rounded-[1.75rem] border border-current/10 bg-current/[0.03] p-8 text-center', className)}>
      ...
      <h2 className="editable-display mt-5 text-2xl font-semibold tracking-[-0.02em]">{title}</h2>
      <p className="mx-auto mt-3 max-w-xl text-sm leading-7 text-current/65">{description}</p>
      <Link href={actionHref} className="mt-6 inline-flex items-center gap-2 rounded-full border border-current/15 px-5 py-3 text-sm font-semibold transition duration-300 hover:border-[var(--slot4-accent)] hover:text-[var(--slot4-accent)]">
        {actionLabel}
