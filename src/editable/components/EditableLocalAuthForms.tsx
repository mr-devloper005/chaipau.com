...
const inputClass = 'h-12 w-full rounded-xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] px-4 text-sm font-medium text-[var(--slot4-page-text)] outline-none transition duration-300 placeholder:text-[var(--slot4-muted-text)] focus:border-[var(--slot4-accent)]'
const buttonClass = 'inline-flex h-12 w-full items-center justify-center rounded-full bg-[var(--slot4-accent)] px-6 text-sm font-semibold text-[var(--slot4-on-accent)] transition duration-300 hover:brightness-95 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-xl px-4 py-3 text-sm font-semibold ${status === 'success' ? 'bg-[var(--slot4-secondary-soft)] text-[var(--slot4-secondary)]' : 'bg-[var(--slot4-accent-soft)] text-[var(--slot4-accent)]'}`}>{message}</p> : null}
