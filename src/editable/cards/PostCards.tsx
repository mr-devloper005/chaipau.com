  ...
  return (
    <Link href={href} className={`group block min-w-0 overflow-hidden rounded-3xl ${dc.motion.lift}`}>
      <div className="relative min-h-[480px] p-6 sm:p-8 lg:min-h-[560px]">
        ...
        <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(29,33,40,0.05),rgba(29,33,40,0.9))]" />
        <div className="relative z-10 flex h-full min-h-[420px] flex-col justify-end text-white lg:min-h-[500px]">
          <span className="inline-flex w-fit items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3.5 py-1.5 text-xs font-semibold uppercase tracking-[0.22em] text-[var(--slot4-accent)] backdrop-blur-sm">{label}</span>
          <h3 className="editable-display mt-5 max-w-3xl text-3xl font-semibold leading-[1.05] tracking-[-0.03em] sm:text-4xl lg:text-5xl">{post.title}</h3>
          <p className="mt-4 max-w-2xl text-sm leading-7 text-white/75 sm:text-base">{getEditableExcerpt(post, 190)}</p>
          <span className="mt-7 inline-flex w-fit items-center gap-2 rounded-full bg-[var(--slot4-accent)] px-5 py-3 text-sm font-semibold text-[var(--slot4-on-accent)] transition duration-300 group-hover:brightness-95">
  ...
  return (
    <Link href={href} className={`group ${dc.layout.minRailCard} block overflow-hidden ${dc.surface.card} ${dc.motion.lift}`}>
      <div className={`${dc.media.frame} ${dc.media.ratio}`}>
        ...
        <span className="absolute left-4 top-4 rounded-full bg-[var(--slot4-dark-bg)] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-white">No. {String(index + 1).padStart(2, '0')}</span>
      </div>
      <div className="p-5">
        ...
        <p className={`mt-3 line-clamp-3 text-sm leading-6 ${pal.softMutedText}`}>{getEditableExcerpt(post, 135)}</p>
      </div>
    </Link>
  )
}
  ...
  return (
    <Link href={href} className={`group block min-w-0 ${dc.surface.soft} p-5 ${dc.motion.lift}`}>
      <div className="flex items-start gap-4">
        <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[var(--slot4-secondary)] text-xs font-semibold text-white">{index + 1}</span>
        <div className="min-w-0">
          ...
          <h3 className={`editable-display mt-2 line-clamp-2 text-lg font-semibold leading-snug tracking-[-0.02em] ${pal.panelText}`}>{post.title}</h3>
  ...
  return (
    <Link href={href} className={`group grid min-w-0 gap-5 overflow-hidden ${dc.surface.card} p-4 ${dc.motion.lift} sm:grid-cols-[220px_minmax(0,1fr)]`}>
      ...
      <div className="min-w-0 p-2 sm:py-4 sm:pr-5">
        ...
        <h2 className={`editable-display mt-3 line-clamp-3 text-2xl font-semibold leading-snug tracking-[-0.02em] ${pal.panelText} sm:text-3xl`}>{post.title}</h2>
